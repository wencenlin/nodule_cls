...
from data.config_training import config
...
def resample(imgs, spacing, new_spacing, order=2):  # 有用到
    if len(imgs.shape) == 3:
...
def worldToVoxelCoord(worldCoord, origin, spacing):  # 有用到
    stretchedVoxelCoord = np.absolute(worldCoord - origin)
    voxelCoord = stretchedVoxelCoord / spacing
    return voxelCoord


def load_itk_image(filename):  # 有用到
    with open(filename) as f:
        contents = f.readlines()
        line = [k for k in contents if k.startswith('TransformMatrix')][0]  # TransformMatrix ：代表x y z的方向，用于判定CT的方向
        transformM = np.array(line.split(' = ')[1].split(' ')).astype('float')
        transformM = np.round(transformM)
        if np.any(transformM != np.array([1, 0, 0, 0, 1, 0, 0, 0, 1])):  # 確認是否影像有翻轉
            isflip = True
        else:
            isflip = False

    itkimage = sitk.ReadImage(filename)  # 讀取影像
    numpyImage = sitk.GetArrayFromImage(itkimage)

    numpyOrigin = np.array(list(reversed(itkimage.GetOrigin())))  # CT原點座標
    numpySpacing = np.array(list(reversed(itkimage.GetSpacing())))  # CT像素間隔

    return numpyImage, numpyOrigin, numpySpacing, isflip


def process_mask(mask):  # 有用到
...
def savenpy_luna(id, annos, filelist, luna_segment, luna_data, savepath):  # 有用到
    islabel = True
    isClean = True
    resolution = np.array([1, 1, 1])
    #     resolution = np.array([2,2,2])
    name = filelist[id]  # name = 1.3.6.1.4.1.14519.5.2.1.6279.6001.317087518531899043292346860596

    sliceim, origin, spacing, isflip = load_itk_image(os.path.join(luna_data, name + '.mhd'))  # 影像、原點座標、像素間隔、是否翻轉

    Mask, origin, spacing, isflip = load_itk_image(os.path.join(luna_segment, name + '.mhd'))  # MASK、原點座標、像素間隔、是否翻轉
    if isflip:
        Mask = Mask[:, ::-1, ::-1]
    newshape = np.round(np.array(Mask.shape) * spacing / resolution).astype('int')  # Uniform Thickness 统一Mask的解析度
    m1 = Mask == 3  # 左肺
    m2 = Mask == 4  # 右肺
    Mask = m1 + m2

    # 求取Mask的bounding box（根据Mask在x，y，z轴上的最大值，最小值）
    # 对掩码求取边界（肺部边界），只保留边界内的数据
    # 想象一个正方形，里面有一个小圆，小圆就是掩码，那么此处做的就是求小圆的最小外接矩形，将矩形外的部分砍掉
    xx, yy, zz = np.where(Mask)
    box = np.array([[np.min(xx), np.max(xx)], [np.min(yy), np.max(yy)], [np.min(zz), np.max(zz)]])  # [[ 38 640][ 88 425][ 16 495]]
    # 对边界即掩码的最小外部长方体应用新解析度
    box = box * np.expand_dims(spacing, 1) / np.expand_dims(resolution, 1)  # [[ 19. 320.][ 53.453125 258.15429688][9.71875 300.67382812]]
    box = np.floor(box).astype('int')  # [[ 19 320][53 258][9 300]]
    # 对这个边界向外扩展一点，为了处理边缘的像素
    margin = 5
    extendbox = np.vstack(
        [np.max([[0, 0, 0], box[:, 0] - margin], 0), np.min([newshape, box[:, 1] + 2 * margin], axis=0).T]).T  # [[ 14 330][ 48 268][4 310]]

    this_annos = np.copy(annos[annos[:, 0] == (name)])

    if isClean:
        convex_mask = m1
        # 采用process_mask函数，分别对左右肺的Mask进行膨胀操作，除肺部的小空洞
        ...
        if isflip:
            sliceim = sliceim[:, ::-1, ::-1]
            print('flip!')
        # 预处理Three automated preprocessing steps are employed for the input CT images:
        # First, we clip the raw data into [−1200, 600].
        # Second, we transform the range linearlyinto [0, 1].
        # Finally, we use LUNA16’s given segmentation ground truth and remove the background.
        sliceim = lumTrans(sliceim)
        # 将膨胀肺区边缘大于210的区域，填充为170
        # 170对应归一化后的水，掩码（膨胀过后）外的区域补充为水
        sliceim = sliceim * dilatedMask + pad_value * (1 - dilatedMask).astype('uint8')
        # 210对应归一化后的骨头，凡是大于骨头的区域都填充为水
        bones = (sliceim * extramask) > bone_thresh
        sliceim[bones] = pad_value

        # 使用resample函数，对其进行重采样
        sliceim1, _ = resample(sliceim, spacing, resolution, order=1)
        # 将原始数据重采样，再截取mask box内的数据
        sliceim2 = sliceim1[extendbox[0, 0]:extendbox[0, 1],
                   extendbox[1, 0]:extendbox[1, 1],
    ...
    if islabel:
        ...
        if len(this_annos) > 0:
            ...
            for c in this_annos:  # ['1.3.6.1.4.1.14519.5.2.1.6279.6001.313334055029671473836954456733' 68.05574564 43.703103399999996 -266.0215516 12.68580657]
                pos = worldToVoxelCoord(c[1:4][::-1], origin=origin, spacing=spacing)  # 转换成体素坐标
                if isflip:
                    pos[1:] = Mask.shape[1:3] - pos[1:]
                label.append(np.concatenate([pos, [c[4] / spacing[1]]]))

        # 转换成体素坐标的coordX,coordY,coordZ,diameter_mm
...
def preprocess_luna():
    # 此過程並無分train set或test set，所有子集合做相同的過程
    # 產生多種.npy
    # 完成後，將所有子集合放在同一個資料夾
    luna_segment = config['luna_segment']  # 'D:/luna16/seg-lungs-LUNA16/'
    savepath = config['preprocess_result_path']  # 'D:/luna16/preprocess/'
    luna_data = config['luna_data']  # 'D:/luna16/data_subset/'
    luna_label = config['luna_label']  # 'D:/luna16/annotations.csv'
