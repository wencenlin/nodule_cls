...
for srcid, lunaantlidc in lunantdictlidc.iteritems():
    ...
    for lunantdictlidcsub in lunaantlidc:
        ...
        if srcid in ['1.3.6.1.4.1.14519.5.2.1.6279.6001.174692377730646477496286081479',
...
for srsid, extant in lunaantdictnodid.iteritems():
    ...
    for extantvlu in extant:
        ...
        for fname in os.listdir(os.path.join(*['/media/data1/wentao/LIDC-IDRI/DOI/', pid, stdid, srsid])):
            if fname.endswith('.xml'):
                dom = xml.dom.minidom.parse(
                    os.path.join(*['/media/data1/wentao/LIDC-IDRI/DOI/', pid, stdid, srsid, fname]))
        ...
        if len(mallst) == 0:
            ...
            vlulst.append(sum(mallst) / float(len(mallst)))
        if len(callst) == 0:
            vlulst.append(0)
        else:
            vlulst.append(sum(callst) / float(len(callst)))
        if len(sphlst) == 0:
            vlulst.append(0)
        else:
            vlulst.append(sum(sphlst) / float(len(sphlst)))
        if len(marlst) == 0:
            vlulst.append(0)
        else:
            vlulst.append(sum(marlst) / float(len(marlst)))
        if len(loblst) == 0:
            vlulst.append(0)
        else:
            vlulst.append(sum(loblst) / float(len(loblst)))
        if len(spilst) == 0:
            vlulst.append(0)
        else:
            vlulst.append(sum(spilst) / float(len(spilst)))
        if len(texlst) == 0:
            vlulst.append(0)
        else:
            vlulst.append(sum(texlst) / float(len(texlst)))
...
antpdframe = pd.read_csv('annotationdetclssgmfnl.csv', names=['seriesuid', 'coordX', 'coordY', 'coordZ', \
                                                              'diameter_mm', 'malignant', 'calcification', 'sphericity',
                                                              'margin', 'lobulation', 'spiculation', 'texture'])
