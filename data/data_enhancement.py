...
path = "D:/luna16/crop_v3/"
dataframe = pd.read_csv("annotationdetclsconvfnl_v3.csv", encoding='utf-8')
...
teidlst = []
for fname in os.listdir('D:\\luna16\\data_subset\\subset9' + '\\'):
    if fname.endswith('.mhd'):
        teidlst.append(fname[:-4])
for data in data_list:
    if data[0].split('-')[0] not in teidlst:  # 表示其data是給training的
...
frame.to_csv('a9.csv', index=None, columns=['seriesuid', 'coordX', 'coordY', 'coordZ', 'diameter_mm', 'malignant'])
