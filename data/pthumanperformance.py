...
# 			prednamelst[fname.split('-')[0]] = [pixdimpred[predidx]]
# 		else:
# 			prednamelst[fname.split('-')[0]].append(pixdimpred[predidx])
# 		predidx += 1
...
for srsid, extant in lunadctclssgmdict.iteritems():
	...
	for subextant in extant:
		...
		for did in range(6, len(subextant),1):#len(subextant), 1):
			...
			if dctptlabel[srsid][did-6] == -1: dctptlabel[srsid][did-6] = 0
# calculate patient level performance
p = [0]*4
n = [0]*4
tp = [0]*4
np = [0]*4
for srs in ptlabel.keys():
	if dctptlabel[srs][0] != -1:
		n[0] += 1
		if ptlabel[srs] == 1: np[0] += 1
		if ptlabel[srs] == dctptlabel[srs][0]: 
			p[0] += 1
			if ptlabel[srs] == 1:
				tp[0] += 1
	if dctptlabel[srs][1] != -1:
		n[1] += 1
		if ptlabel[srs] == 1: np[1] += 1
		if ptlabel[srs] == dctptlabel[srs][1]: 
			p[1] += 1
			if ptlabel[srs] == 1:
				tp[1] += 1
	if dctptlabel[srs][2] != -1:
		n[2] += 1
		if ptlabel[srs] == 1: np[2] += 1
		if ptlabel[srs] == dctptlabel[srs][2]: 
			p[2] += 1
			if ptlabel[srs] == 1:
				tp[2] += 1
	if dctptlabel[srs][3] != -1:
		n[3] += 1
		if ptlabel[srs] == 1: np[3] += 1
		if ptlabel[srs] == dctptlabel[srs][3]: 
			p[3] += 1
			if ptlabel[srs] == 1:
				tp[3] += 1
print(p, n, tp, np, len(ptlabel))
print(p[0]/float(n[0]), tp[0]/float(np[0]), (p[0]-tp[0])/float(n[0]-np[0]))
print(p[1]/float(n[1]), tp[1]/float(np[1]), (p[1]-tp[1])/float(n[1]-np[1]))
print(p[2]/float(n[2]), tp[2]/float(np[2]), (p[2]-tp[2])/float(n[2]-np[2]))
print(p[3]/float(n[3]), tp[3]/float(np[3]), (p[3]-tp[3])/float(n[3]-np[3]))
import numpy as np 
np.save('ptlabel'+str(fold)+'.npy', ptlabel)
np.save('dctptlabel'+str(fold)+'.npy', dctptlabel)